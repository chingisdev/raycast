...
/*   Updated: 2021/03/10 16:32:38 by mvernius         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	all->textures.north.w_img.img = mlx_xpm_file_to_image(all->win.mlx,
		all->info.north_path, &(all->textures.north.width),
		&(all->textures.north.height));
	all->textures.south.w_img.img = mlx_xpm_file_to_image(all->win.mlx,
		all->info.south_path, &(all->textures.south.width),
		&(all->textures.south.height));
	all->textures.west.w_img.img = mlx_xpm_file_to_image(all->win.mlx,
		all->info.west_path, &(all->textures.west.width),
		&(all->textures.west.height));
	all->textures.east.w_img.img = mlx_xpm_file_to_image(all->win.mlx,
		all->info.east_path, &(all->textures.east.width),
...
void		ft_init_all_textures(t_all *all)
{
	...
	if (all->textures.east.w_img.addr == NULL)
		ft_close_if_error("Error:\neast addr isn't created\n");
	if (all->textures.north.w_img.addr == NULL)
		ft_close_if_error("Error:\nnorth addr isn't created\n");
	if (all->textures.west.w_img.addr == NULL)
		ft_close_if_error("Error:\nwest addr isn't created\n");
	if (all->textures.south.w_img.addr == NULL)
		ft_close_if_error("Error:\nsouth addr isn't created\n");
}
